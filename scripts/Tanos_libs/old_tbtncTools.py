...
def read_fullTSHS(HSfilename, geomFDFfilename):
    ...
    geomFDF = si.get_sile(geomFDFfilename).read_geometry()
    # Update species
    for ia, (a, afdf) in enumerate(zip(HSfile.atom, geomFDF.atom)):
        A = si.Atom(afdf.Z, a.orbital)
        HSfile.atom[ia] = A
    HSfile.reduce()
    return HSfile
...
def get_R_hop(tshs, tbt, xyz_tip, pzidx, nn, return_S=False):
    a_dev = tbt.a_dev
    tshs_dev = tshs.sub(a_dev)
    C_list = (tshs_dev.xyz[:, 2] == tshs_dev.xyz[0, 2]).nonzero()[0]
    # Check that we have selected only carbon atoms
    for ia, a in zip(C_list, tshs_dev.atom[C_list]):
        if a.Z != 6:
            print(ia, tshs_dev.xyz[ia])
            print('WARNING: cannot build TB model because the provided geometry\n\tis not a pristine graphene')
            exit(1)
...
def Delta(TSHS, HS_TB, shape='Cuboid', ext_offset=None, center=None, thickness=None):
    # Center of shape in TSHS 
    if center is None:
        center = TSHS.center(atom=(TSHS.xyz[:,2] == TSHS.xyz[0,2]).nonzero()[0])
    # Thickness in Ang
...
def makeTB(TSHS_0, pzidx, nn, WW, LL, TSHS_elec, save=True, return_bands=False):
    """
    ...
    # ELECTRODE
    n_el = int(round(TSHS_elec.cell[1,1]/H0.cell[1,1]))
...
def makeTB_FrameOutside(tshs, tbt, xyz_tip, TSHS_0, pzidx, nn, WW, LL, TSHS_elec, 
    ...
    print('r ({}; Angstrom)\t Final parameters ({}; eV):'.format(len(ref_r), len(hop_atframe)))
    ...
    H0.construct((ref_r+dR, zip(hop_atframe, ref_over)))
...
def func_smooth(x, y, first_x, second_x, y1, delta=8):
    ...
    if first_x > np.amax(x):
        idx = (np.abs(x - np.amax(x)) < 3.).nonzero()[0]
    else:
        idx = (np.abs(x - first_x) < 3.).nonzero()[0]
    y0 = np.average(y[idx])
...
def makeTB_fermi(tshs, tbt, xyz_tip, frameOrigin, TSHS_0, pzidx, nn, 
    WW, LL, TSHS_elec, save=True, cut_R=None, smooth_R=15., return_bands=False):
    """
    tshs:           TSHS object from "dirty graphene" calculation
    tbt:            tbtncSile object from tbtrans calculation with HS: "tshs"
...
def plot_bondcurrents(f, idx_elec, sum='+', E=0.0,  k='avg', avg=True, percent_bg=False, 
