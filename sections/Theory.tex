...
\begin{figure}[H]
	\begin{center}
		\begin{tikzpicture}
			...
			\orbital[pos = {(6,6.46)}]{-pz}
			\node[above] at (6,7.46) {-p$_\pi$};
			\orbital[pos = {(4,6.46)}]{pz}
			\node[above] at (4,7.46) {p$_\pi$};
			\draw[dashed, very thick] (6,6.46) -- (4,6.46);
			\draw[dashed, very thick] (7,4.73) -- (6,6.46);
			\draw[dashed, very thick] (4,6.46) -- (3,4.73);
			\draw[dashed, very thick] (6,3) -- (7,4.73);
			\draw[dashed, very thick] (3,4.73) -- (4,3);
			\orbital[pos = {(4,3)}]{pz}
			\node[above] at (4,4) {p$_\pi$};
			\orbital[pos = {(6,3)}]{-pz}
			\node[above] at (6,4) {-p$_\pi$};
			\draw[dashed, very thick] (4,3) -- (6,3);
		\end{tikzpicture}
...
\end{align}
where \(\phi_{\alpha}(R)\) is some atomic orbital at position \(R\), with \(\alpha\) denoting a specific valence orbital. In electron transport the states close to the Fermi level is of interest. These are namely the highest occupied moelcular orbitals (HOMO), or the lowest unoccupied molecular orbitals (LUMO).
