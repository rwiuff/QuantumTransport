...
\begin{figure}[ht]
	...
	\label{2DTranssys}
\end{figure}
This system contains the 1D semi-infinite chain of cells however, when describing cells in the added dimension, one simply needs to shift the system by a phase shift. There are two considerations: Firstly the range of \textit{k}-points exists between \(-\pi\) and \(\pi\). Secondly these are periodic boundaries. One really needs to pick a range of perpendicular \textit{k}-points of interest and then the idea is to construct a Hamiltonian for the 1D chain, located at that point. The recursion routine then calculates Green's functions and transmission. The Hamiltonian is constructed using the function \textit{PeriodicHamiltonian}. This function uses the equation:
\begin{align}
