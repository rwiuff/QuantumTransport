...
\end{align}
Here it have been utilised that the Green's matrix is asymmetric in time (\(\mathbf{G}=\mathbf{G}^{T}\), \(\mathbf{G}^{T}\mathbf{G}=\mathbf{I}\), \(\mathbf{G}^{T}=\mathbf{G}^-1\)) as well as the fact that \(\mathbf{G} = z\mathbf{1}-\mathbf{H}-\mathbf{\Sigma}\).
