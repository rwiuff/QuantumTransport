...
\begin{align}
    \mathbf{A} &= i(\mathbf{G}-\mathbf{G}^{\dagger})\\
    &= i(\mathbf{G}^{\dagger}\left[\mathbf{G}^{\dagger}-\mathbf{G}\right]\mathbf{G})\nonumber\\
    &= i(\mathbf{G}^{\dagger}[E-\mathbf{H}^{\dagger}-\mathbf{\Sigma}^{\dagger} - (E -\mathbf{H}-\mathbf{\Sigma})]\mathbf{G})\nonumber\\
    &=i(\mathbf{G}^{\dagger}[-\mathbf{\Sigma}^{\dagger}+\mathbf{\Sigma})]\mathbf{G})\nonumber\\
    &=i(\mathbf{G}^{\dagger}\mathbf{\Gamma}\mathbf{G})
\end{align}
Here it have been utilised that the Green's matrix is asymmetric in time (\(\mathbf{G}=\mathbf{G}^{T}\), \(\mathbf{G}^{T}\mathbf{G}=\mathbf{I}\), \(\mathbf{G}^{T}=\mathbf{G}^-1\)) as well as the fact that \(\mathbf{G} = z\mathbf{1}-\mathbf{H}-\mathbf{\Sigma}\). Additionally the Hamiltonians cancel out as they are unitary.
The definition of \mathbf{A} is that it is a \textit{spectral function}. It is the change of the imaginary Green's functions. As seen in the equations above it can be described by the Green's matrix and the rate matrix. As mentioned in the previous sections the imaginary part of the Green's functio
