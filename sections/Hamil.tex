%!TEX root = ../Main.tex
\subsubsection{Creating on-site Hamiltonian and hopping matrices for graphene systems}
...
\captionof{listing}{The outer operator in numpy is manifested as two nested loops. On lines 35-37 each atomic distance is calculated. Line 38 replaces all nearest neighbour distances with an input potential, leaving the rest as zero. Lastly the diagonal is subtracted from the matrix.\label{npouter}}\vspace{\baselineskip}
...
\begin{figure}[H]
	...
	\label{atomrepfig}
\end{figure}
\subsubsection{Defining the full Hamiltonian and solving the Schr\"{o}dinger equation}\label{FullHam}
...
\vspace{-1\baselineskip}
\captionof{listing}{Function producing the full hamiltonian, corresponding to \cref{hamileq}.\label{fullhamil}}\vspace{\baselineskip}
\subsubsection{Plotting the band structures}
