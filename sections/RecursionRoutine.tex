...
As a good way to check whether the recursion routine has been implemented successfully, the real and imaginary part of the Green's function can be plotted. With a relatively simple approach, the Green's function can be obtained as a function of energy, using a \text, looping over a range of energies which is then used as input in the function \cref{recursionfunc} (see \cref{plotcode}):
\begin{listing}[ht]
