...
\im{Listings/Development.py}{14}{18}\\
...
\begin{flalign}
\begin{split}
\vb{H} = &\vb{h}_0 + (\vb{V}_{1(x)}e^{-ik_x} + \vb{V}_{1(x)}^{\dagger}e^{ik_x} + \vb{V}_{2(x)}e^{-ik_y} + \vb{V}_{2(x)}^{\dagger}e^{-ik_y}\\ & + \vb{V}_{3(xy)}e^{-ik_x}e^{-ik_y} + \vb{V}_{3(xy)}^{\dagger}e^{ik_x}e^{ik_y})
\end{split}
\end{flalign}
...
\subsection{Plotting the bandstructures}
