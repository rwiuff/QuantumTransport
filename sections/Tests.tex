...
\subsection{Test 1}
The first test will be manipulation of the system 'PS4O'. The basis structure is para-NPG where 4 oxygen sites have been added, two on each benzene ring (see \cref{appfigs},\cref{Strucow}, nr.x). Practically carbon atoms added instead of oxygen. They will be manipulated by lowering their on-site potential. This is to simulate adding oxygen. In \cref{PS4O} an overview of the structure can be seen along with the band structure for the system, obtained through DFT and TBtrans. 
\begin{figure}[h]
    \centering
...
\subsection{Test 2}
Next test will be with the 'PS4OH' system. Again the basis structure is para-NPG, but instead of four oxygen atoms added it will by hydroxide group (see \cref{appfigs}, \cref{Strucow}, nr. x). Here the test is to show the difference between removing a OH site entirely (effectively the hydrogen in hydroxide removes the extra pi-electron in oxygen from the system, which can be simulated by removing the atoms entirely) and lowering the on-site potential of the oxygen in the OH group significantly in relation to the rest of the system. Again the results from DTF/Tbtrans is shown first in \cref{somehydroxidefigure} and afterwards results from developed programs. 
\begin{figure}[h]
    \centering
    \begin{subfigure}[b]{0.5\textwidth}
    \centering
    ...
    \label{PS4O}
\end{figure}
Multiple on-site potentials were tested to see which one came closest to that of the band structures in \cref{}. 
\begin{figure}[h]
    \centering
    \begin{subfigure}[b]{0.3\textwidth}
    \centering
    ...
    \label{PS4Odev}
\end{figure}
It was found that using values closest to that given by the DFT calculation (\SI{xx}{\electronvolt}) yielded the best results. 
\subsection{Test 3}
\subsection{Test 4}
\subsection{Test 5}
\subsection{Test 6}
